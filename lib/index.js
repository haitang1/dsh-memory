...
export function apply(ctx, config = {}) {
  ...
  if (tools !== undefined) {
    for (const definition of toolDefinitions(store, resolved, requestConsolidation, runtimeStats)) {
      try {
...
function toolDefinitions(store, resolved, requestConsolidation, runtimeStats) {
  ...
  return [
    ...
    defineTool({
      ...
      async execute(_args, exec) {
        ...
        const [summary, raw, persisted, journal, summaryBytes, rawBytes, rolloutFiles] = await Promise.all([
          store.readSummary(),
