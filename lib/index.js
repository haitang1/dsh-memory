...
export function apply(ctx, config = {}) {
  ...
  if (settings !== undefined) {
    settingsScope = settings.register(settingsNamespace('memory'), Config, {
      base: config,
      applies: 'live'
    })
  ...
  // Web settings page backend: a same-origin GET/POST endpoint the browser
  // card uses to read and save the memory namespace (the apiproxy settings
  // boundary is a level above plugins, so the card talks to this route).
  const webServer = ctx.get('webServer')
  if (webServer !== undefined && settings !== undefined && settingsScope !== null) {
    disposers.push(webServer.register({
      kind: 'exact',
      path: '/_dsh/memory/settings',
      handler: memorySettingsRouteHandler(ctx, settings, settingsScope)
    }))
  }
  ...
  const telemetry = { errorCount: 0, lastError: null }
