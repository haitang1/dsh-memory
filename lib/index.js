...
export function apply(ctx, config = {}) {
  ...
  // observable without a tool call (diagnostics.json).
  ...
  if (tools !== undefined) {
    ...
    try {
      ...
      ctx.logger.error('dsh-memory: toolDefinitions failed: %o', error)
      definitions = []
    }
    ...
    writeStartupDiagnostics({ at: Date.now(), toolsService: false, note: 'tools service unavailable' })
  }
