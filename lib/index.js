    ...
    defineTool({
      ...
      output: {
        ...
        render: (_args, value) => textOutput([
          ...
          `scopes (${value.scopeCount}): ${value.scopes.map((item) => `${item.key}(raw ${item.rawCount}, v${item.summaryVersion}, cursor ${item.journalCursor}${item.consolidating ? ', consolidating' : ''})`).join('; ') || 'none'}`,
