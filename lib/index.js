...
import { AUTO_MEMORY_SKILL, extractMessageText, resolveSummarizeRoute } from './automation.js'
export { AUTO_MEMORY_SKILL, extractMessageText, resolveSummarizeRoute } from './automation.js'
...
const DEFAULT_CONSOLIDATE_MAX_TOKENS = 8192
const DEFAULT_LLM_RETRIES = 1
const DEFAULT_MAX_ACTIVE_SUMMARIES = 4

// Consolidation must be able to emit the whole bounded summary, so its token
// budget has a floor derived from the byte budget. Without it, a user-layer
// value inherited from an older release (consolidateMaxTokens: 3000) caps the
// merge below the size this plugin asks the model to produce, and every
// consolidation fails with 'LLM output reached max tokens' while the rest of
// the plugin still looks healthy. Larger configured values are honored; only
// sub-floor values are lifted.
const MIN_CONSOLIDATE_MAX_TOKENS = 4096
const CONSOLIDATE_BYTES_PER_TOKEN = 2
const MAX_CONSOLIDATE_MAX_TOKENS = 16384

/** Lowest safe output-token budget for a summary bounded at `maxBytes` bytes. */
function consolidateTokenFloor(maxBytes) {
  const needed = Math.ceil(maxBytes / CONSOLIDATE_BYTES_PER_TOKEN)
  return Math.min(MAX_CONSOLIDATE_MAX_TOKENS, Math.max(MIN_CONSOLIDATE_MAX_TOKENS, needed))
}
...
function resolveConfig(config = {}) {
  const merged = { ...config }
  const maxBytes = Number.isFinite(merged.maxBytes) && merged.maxBytes > 0 ? merged.maxBytes : DEFAULT_MAX_BYTES
  const configuredConsolidateMaxTokens = Number.isFinite(merged.consolidateMaxTokens) && merged.consolidateMaxTokens > 0 ? merged.consolidateMaxTokens : DEFAULT_CONSOLIDATE_MAX_TOKENS
  const consolidateMaxTokens = Math.max(configuredConsolidateMaxTokens, consolidateTokenFloor(maxBytes))
  const configAlerts = consolidateMaxTokens === configuredConsolidateMaxTokens
    ? []
    : [{
        key: 'consolidateMaxTokens',
        configured: configuredConsolidateMaxTokens,
        effective: consolidateMaxTokens,
        reason: 'configured value is below the token budget needed to emit a maxBytes-sized summary; raise it in the Memory settings card or remove the user-layer override'
      }]
  return {
...
function extractTurnText(agent, fromSeq) {
  let text = ''
  let lastSeq = fromSeq
  for (const event of agent.session.snapshotEvents(fromSeq)) {
    const seq = event.seq
...
/**
 * Attach the effective token budget to a truncation failure. The raw message
 * ('LLM output reached max tokens') names neither the key nor its value, which
 * is what makes an inherited user-layer override so hard to diagnose: the
 * plugin keeps loading and serving tools while every merge quietly fails.
 */
function budgetHint(error, resolved, kind) {
  const message = error instanceof Error ? error.message : String(error)
  if (!message.includes('max tokens')) return error
  const key = kind === 'consolidate' ? 'consolidateMaxTokens' : 'summaryMaxTokens'
  return new Error(`${message} (${key}=${resolved[key]}; raise it in the Memory settings card or remove the user-layer override)`, { cause: error })
}
...
export function apply(ctx, config = {}) {
  ...
  async function routeScope(args, exec) {
    ...
    if (!resolved.scopedMemory) {
      // Scoped memory is off: degrade explicit workspace/project requests to
      // the global store instead of failing, so agents following the tools'
      // "workspace (default)" wording never hit a hard error.
  ...
  ctx.inject(['settings'], (settingsCtx) => {
    ...
    const settingsScope = settings.register('memory', Config, {
      base: config,
      applies: 'live'
    })
    settingsCtx.effect(() => {
      ...
      const applySettings = (next) => {
        ...
        // A lifted budget is a real (if recoverable) misconfiguration: surface it
        // where a human will see it instead of letting the merge look healthy.
        if (resolved.configAlerts.length > 0) {
          ctx.logger.warn('dsh-memory: %s', resolved.configAlerts.map((alert) => `${alert.key}=${alert.configured} is below the safe minimum; using ${alert.effective}`).join('; '))
        }
        writeStartupDiagnostics({ at: Date.now(), configAlerts: resolved.configAlerts })
      }
      applySettings(settingsScope.get())
      return settingsScope.watch(applySettings)
    }, 'dsh-memory: settings watch')
    installMemorySettingsWeb(settingsCtx, settings, () => Config({}))
  })
  ...
  // The llm service may be registered by the harness after this bundle boots:
  // a plain ctx.get('llm') returns undefined on DSH 0.1.2-rc.1, silently
  // disabling every automatic summarization with 'disabled' skips. Wait for
  // the service instead, like the tools/skills registrations above.
  let llm
  ...
  async function runSummarize(agent, text, lastSeq) {
    ...
    try {
      ...
      recordError('summarize', budgetHint(error, resolved, 'summarize'))
      ctx.logger.warn('dsh-memory: turn summarization failed: %o', budgetHint(error, resolved, 'summarize'))
    }
  }
  ...
  async function consolidateNow(runtime) {
    ...
    try {
      ...
      recordError('consolidate', budgetHint(error, resolved, 'consolidate'))
      ctx.logger.warn('dsh-memory: consolidation failed: %o', budgetHint(error, resolved, 'consolidate'))
    }
  }
    ...
    defineTool({
      ...
      output: {
        schema: {
          ...
          properties: {
            ...
            summaryMaxTokens: { type: 'number', required: true },
            consolidateMaxTokens: { type: 'number', required: true },
            configAlerts: {
              type: 'array',
              required: true,
              items: {
                ...
                properties: {
                  ...
                  configured: { type: 'number', required: true },
      ...
      async execute(_args, exec) {
        ...
        return {
          ...
          consolidateMaxTokens: resolved.consolidateMaxTokens,
          configAlerts: resolved.configAlerts ?? [],
