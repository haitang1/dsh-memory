...
export function apply(ctx, config = {}) {
  ...
  function routeScope(args, exec) {
    const explicit = normalizeScopeArg(args && args.scope)
    if (explicit === 'global') return { key: 'global', store }
    if (!resolved.scopedMemory) {
      if (explicit === 'workspace') throw new Error('memory: scopedMemory is disabled')
      return { key: 'global', store }
    }
    const key = scopeFromSession(exec && exec.agent && exec.agent.session)
    if (key === 'global') return { key: 'global', store }
    ...
    disposers.push(systemPrompt.context({
      ...
      text: (context) => {
        try {
          ...
          if (scopedText.trim().length === 0) return globalPart
  ...
  if (tools !== undefined) {
    for (const definition of toolDefinitions(store, resolved, requestConsolidation, runtimeStats, stateRef, routeScope)) {
      try {
...
function toolDefinitions(store, resolved, requestConsolidation, runtimeStats, stateRef, routeScope) {
  ...
  return [
    defineTool({
      ...
      parameters: {
        scope: { type: 'string', description: "Memory scope: 'global' or 'workspace' (default: current session workspace when scopedMemory is enabled, otherwise global)." },
      },
      output: {
        schema: {
          type: 'object',
      ...
      async execute(args, exec) {
        ...
        const route = routeScope(args, exec)
    ...
    defineTool({
      ...
      async execute(args, exec) {
        ...
        if (route.key === 'global') requestConsolidation()
        return result
      }
    }),
    defineTool({
    ...
    defineTool({
      ...
      async execute(args, exec) {
        ...
        const tags = Array.isArray(args && args.tags)
          ...
          : []
        const hits = await route.store.searchRaw(query, { limit, tags, mode })
