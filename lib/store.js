...
/**
 * Heuristic credential detection: well-known key shapes plus high-entropy
 * alphanumeric tokens. Used to refuse obvious secrets in memory_add and to
 * redact injected summaries. It is best-effort, not a replacement for real
 * secret scanning.
 */
export function detectSecrets(text) {
  const input = String(text ?? '')
  const findings = []
  const push = (type, matched, index) => {
    if (findings.length >= 20) return
    findings.push({ type, matched, index })
  }
  const patterns = [
    { type: 'aws-access-key', regex: /\bAKIA[0-9A-Z]{16}\b/g },
    { type: 'github-token', regex: /\bgh[pousr]_[A-Za-z0-9]{20,255}\b/g },
    { type: 'openai-key', regex: /\bsk-[A-Za-z0-9_-]{16,}\b/g },
    { type: 'private-key', regex: /-----BEGIN (?:RSA |EC |OPENSSH |DSA )?PRIVATE KEY-----/g },
    { type: 'credential-assignment', regex: /\b(?:token|secret|password|api[_-]?key)\s*[:=]\s*["']?([A-Za-z0-9_\-./+]{12,})/gi }
  ]
  for (const { type, regex } of patterns) {
    for (const match of input.matchAll(regex)) {
      const matched = match[1] !== undefined ? match[1] : match[0]
      push(type, matched, match.index)
    }
  }
  const tokenPattern = /[A-Za-z0-9_\-]{20,128}/g
  for (const match of input.matchAll(tokenPattern)) {
    if (shannonEntropy(match[0]) >= 4.2) push('high-entropy-token', match[0], match.index)
  }
  return findings
}

export function redactSecrets(text, findings = detectSecrets(text)) {
  let output = String(text ?? '')
  for (const finding of findings) {
    output = output.split(finding.matched).join(`[REDACTED:${finding.type}]`)
  }
  return output
}
...
/**
 * Deterministic feature-hashing embedding over word tokens and character
 * n-grams (2 and 3). This is a zero-dependency local vectorizer, not a neural
 * embedding model: it captures lexical overlap/typo-tolerant similarity and
 * provides cosine retrieval without any external service.
 */
export function localEmbedding(text, dimensions = 256) {
  const vector = new Float64Array(dimensions)
  const input = String(text ?? '').toLowerCase()
  const features = tokenizeQuery(input)
  for (let index = 0; index + 1 < input.length; index += 1) features.push(`2:${input.slice(index, index + 2)}`)
  for (let index = 0; index + 2 < input.length; index += 1) features.push(`3:${input.slice(index, index + 3)}`)
  for (const feature of features) {
    const bucket = hashFeature(feature) % dimensions
    const sign = (hashFeature(`${feature}:s`) & 1) === 0 ? 1 : -1
    vector[bucket] += sign
  }
  let norm = 0
  for (let index = 0; index < dimensions; index += 1) norm += vector[index] * vector[index]
  norm = Math.sqrt(norm)
  if (norm > 0) for (let index = 0; index < dimensions; index += 1) vector[index] /= norm
  return vector
}
...
export function vectorSearchEntries(entries, query, { dimensions = 256, topK = 20, threshold = 0.15 } = {}) {
  const queryVector = localEmbedding(query, dimensions)
  const scored = []
  for (const entry of entries) {
    const similarity = cosineSimilarity(queryVector, localEmbedding(entry.content, dimensions))
    if (similarity >= threshold) scored.push({ entry, score: similarity })
  }
  scored.sort((a, b) => b.score - a.score || String(b.entry.ts).localeCompare(String(a.entry.ts)))
  return scored.slice(0, Math.max(1, Math.trunc(topK)))
}
 ...
 */
export function searchEntries(entries, query, { tags = [], mode = 'all', limit = 10, now = Date.now(), fuzzy = true, vector = false } = {}) {
  ...
  for (const entry of entries) {
    ...
    const exactAll = terms.every((term) => result.hitTerms.has(term))
    const matched = effectiveMode === 'any'
      ? result.hitTerms.size > 0
      : exactAll
    if (vector && !matched) {
      const vectorHits = vectorSearchEntries([entry], query, { topK: 1, threshold: 0.3 })
      if (vectorHits.length > 0) {
        scored.push({ entry, score: result.score + vectorHits[0].score * 8, matchedTerms: result.hitTerms.size })
      }
      continue
    }
    if (!matched) continue
  ...
  if (vector) {
    const vectorHits = vectorSearchEntries(entries, query, { topK: maxResults, threshold: 0.3 })
    for (const hit of vectorHits) {
      const existing = scored.find((item) => item.entry.id === hit.entry.id)
      if (existing === undefined) scored.push({ entry: hit.entry, score: hit.score * 8, matchedTerms: 0 })
      else existing.score += hit.score * 4
    }
  }
...
/**
 * Remote embedding retrieval over an OpenAI-compatible `/embeddings` endpoint.
 * The remote cosine ranking is merged with the local BM25 result set: remote
 * candidates that already matched get a score boost, remote-only candidates
 * enter by their vector score.
 */
export async function searchEntriesRemote(entries, query, options = {}) {
  const { tags = [], mode = 'all', limit = 10, now = Date.now(), fuzzy = true, embedding = undefined, threshold = 0.3 } = options
  if (embedding === undefined || !embedding.baseURL) {
    return searchEntries(entries, query, { tags, mode, limit, now, fuzzy, vector: false })
  }
  const normalizedTags = Array.isArray(tags)
    ? tags.filter((tag) => typeof tag === 'string').map((tag) => tag.trim().toLowerCase()).filter(Boolean)
    : []
  ...
  const vectors = await requestEmbeddings([query, ...filtered.map((entry) => entry.content)], embedding)
  const queryVector = vectors[0]
  const remote = filtered
    .map((entry, index) => ({ entry, score: cosineSimilarity(queryVector, vectors[index + 1]) }))
    .filter((item) => item.score >= threshold)
    .sort((a, b) => b.score - a.score || String(b.entry.ts).localeCompare(String(a.entry.ts)))

  const lexical = searchEntries(filtered, query, { tags: [], mode, limit, now, fuzzy, vector: false })
  const boosted = new Map()
  for (const item of lexical) boosted.set(item.entry.id, item.score)
  for (const item of remote) {
    const id = item.entry.id
    if (boosted.has(id)) boosted.set(id, boosted.get(id) + item.score * 4)
    else boosted.set(id, item.score * 8)
  }
  return [...boosted.entries()]
    ...
    .sort((a, b) => b.score - a.score || String(b.entry.ts).localeCompare(String(a.entry.ts)))
    .slice(0, Math.min(Math.max(1, Math.trunc(limit) || 10), 50))
}
...
`, 'utf8')
  ...
  async searchRaw(query, options = {}) {
    ...
    if (options.vector && options.embedding !== undefined && options.embedding.baseURL) {
      return searchEntriesRemote(combined, query, { ...options, limit })
    }
